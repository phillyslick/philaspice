module FrontHelper
  ...
  def mobile_image(image_name, image_title = "")
    if image_name.include?('.') 
      stripped_name = image_name.match(/^[^\.]*/) 
      extension = image_name.match(/([a-zA-Z0-9\s]+)$/) 
    end
    
    if mobile?
      (image_tag "#{stripped_name}_mobile.#{extension}", alt: image_title, title: image_title)
    else
      (image_tag image_name, alt: image_title, title: image_title) 
    end
  end
